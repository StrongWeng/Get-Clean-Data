analyse=function(){
## Step 1:First read in variables and activity names we need for 
##        merging latter.
variables=read.table("./UCI HAR Dataset/features.txt",sep="")
activity=read.table("./UCI HAR Dataset/activity_labels.txt",sep="")

## Read in Test group data,including respondants,activity 
##        name codes and every dimensions of activity measured.
...
ytest=read.table("./UCI HAR Dataset/test/Y_test.txt",sep=""
                 ,col.names="Activity")
## Extracts only the measurements of mean and standard 
...
## Merge the separate data into one test data.
Test=cbind(testsubject,ytest,xtest1,xtest2)

## Read in Train group data as Step 2.
...
## sMerge test group and train group to form a complete data.
...
tidydata=NULL
for(i in 3:81){
tidydata=cbind(tidydata,tapply(TestTrain[,i]
    ,interaction(TestTrain$Subject,TestTrain$Activity),mean))}
tidydata=as.data.frame(tidydata)
colnames(tidydata)=tobereplaced[3:81]
mainnames=rownames(tidydata)
main=strsplit(mainnames,"\\.");main=as.data.frame(main)
Subject=t(main[1,])
Activity=t(main[2,])
